...
class ReportGenerator:
    def generate_report(self):
        ...
        dangerous_emails = session.query(Email).filter(Email.is_dangerous == True).count()
        safe_emails = total_emails - dangerous_emails

        total_attachments = session.query(Attachment).count()
        dangerous_attachments = session.query(Attachment).filter(Attachment.is_dangerous).count()
